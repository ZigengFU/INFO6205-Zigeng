...
public class UF_HWQUPC implements UF {
    ...
    private void mergeComponents(int i, int j) {
        ...
        if (height[i] == height[j]) { // increase the height by one (if the two tree are the same height)
            updateParent(j,i);
            height[i] += 1;
        }
        else if(height[i] > height[j]){ // leave the tree heights unchanged (if the two tree have different heights)  
